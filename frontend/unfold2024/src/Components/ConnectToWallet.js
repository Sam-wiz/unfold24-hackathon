...
function ConnectToWallet() {
  const { userAddress, setUserAddress,setAuthenticated,setUserId } = useContext(AuthContext);
      ...
      try {
        ...
        try {
          const res = await fetch(
            `${process.env.REACT_APP_BACKEND_URL}/users/wallet/${accounts[0]}`,
            {
              method: "GET",
              headers: {
          ...
          if (data.exists) {
              ...
              toastSuccess("Wallet data fetched successfully!");
          } 
        } catch (error) {
  ...
  const signTransaction = async () => {
    ...
    try {
      ...
      const tx = {
        ...
        value: 10000000000, // Amount in ETH
        gasLimit: 21000,
        gasPrice: 10000,
      };
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      !userAddress ? (
        <>
        <button type="button" onClick={btnhandler} class="text-gray-900 bg-white hover:bg-gray-100 border border-gray-200 focus:ring-4 focus:outline-none focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-gray-600 dark:bg-gray-800 dark:border-gray-700 dark:text-white dark:hover:bg-gray-700 me-2 mb-2">
...
Connect with MetaMask
        ...
        <div className="flex flex-col items-center">
          <p className="text-green-500">Wallet Connected: {userAddress}</p>
          <button
